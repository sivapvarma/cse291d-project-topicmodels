...
print('going ahead')
settings_file = 'settings.example'
outfile = 'newsgroups_50'
loss = 'L2'
K = 40 # topics to learn

params = Params(settings_file)
# params.dictionary_file = vocab_file
# M = scipy.io.loadmat(infile)['M']

docword_mat_fname = 'M_newsgroups_full_docs_v_1000.mat'
matrix = scipy.io.loadmat(docword_mat_fname)['M']
print(type(matrix))
matrix = matrix.tocsr().astype(int)
print(type(matrix))
print(matrix.shape)
M = matrix.T # transpose to get scipy format
