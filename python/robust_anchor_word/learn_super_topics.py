...
import time
from Q_matrix import generate_Q_matrix, rectify
import scipy.io
...
Q_unrectified = generate_Q_matrix(M)
# rectify Q(C)
print("rectifying Q now")
Q = rectify(Q_unrectified, K)
...
## learn super topics of topics
# SK = K // 3
SK = 3


R_matrix_name = outfile+".R"
scipy.io.savemat(R_matrix_name, {'M' : R}, oned_as='column')

print("Done saving R matrix")
# candidate_super_anchors = []

#only accept anchors that appear in a significant number of docs
# for i in range(R.shape[0]):
#     if len(np.nonzero(R[i, :])[1]) > params.anchor_thresh:
#         candidate_anchors.append(i)
